...
import { API_ROOT, API_URL } from '@/utils/apiBase';
...
export default function ProductPageClient({ slug, initialProduct }: ProductPageClientProps) {
  ...
  // El producto llega renderizado desde el servidor; sólo se refetchea si
  // cambia el idioma o si el servidor no pudo resolverlo.
  useEffect(() => {
    if (!slug) return;
    if (initialProduct && locale === 'es') {
      trackProductView({
        id: initialProduct.id,
        nombre: initialProduct.nombre,
        precio: parseFloat(initialProduct.precio),
        categoria: initialProduct.categoria?.nombre,
      });
      fbPixel.viewContent(
    ...
    const fetchProduct = async () => {
      try {
        const id = initialProduct?.id ?? slug;
        const res = await fetch(`${API_URL}/catalogo/productos/${id}/?lang=${locale}`, {
          method: 'GET',
